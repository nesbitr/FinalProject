...
int numShots;
boolean shooting;
public static boolean classic, insanity = false;
...
public void setUpNextLevel(){ //start new level with one more asteroid
 ...
 if (insanity){
	 ship=new Ship(250,250,0,.45,.98,.2,8);
	 ...
	 maxAstVel,astNumHits,astNumSplit,classic);
 }
 if (classic&&!insanity){
	 ship=new Ship(250,250,0,.45,.98,.3,8);
	 numShots=0; //no shots on the screen at beginning of level
	 paused=false;
	 shooting=false;
	 //create an array large enough to hold the biggest number
 ...
 if (level>=3){
	 ship=new Ship(250,250,0,.35,.98,.15,1);
 }
 if (level>=4){
	 ship=new Ship(250,250,0,.35,.98,.2,1);
 }
 if (level>=5){
	 ship=new Ship(250,250,0,.35,.98,.3,1);
 }
 if (level>=6){
	 ship=new Ship(250,250,0,.35,.98,.45,1);
 }
 numShots=0; //no shots on the screen at beginning of level
 paused=false;
 shooting=false;
...
if (lives>0){ 
 ...
 if (insanityRef1!=insanityRef2){
		if (x==0){
		 g.setColor(Color.green);
		 x=1;
		insanityRef2=insanityRef1;
		}
		else{
 ...
 g.drawString("Lives: "+ lives, 430, 20);
...
public Asteroid(double x,double y,double radius,double minVelocity,
 double maxVelocity,int hitsLeft,int numSplit, boolean classic1){
 this.x=x;
 this.y=y;
...
if (classic1){
 g.setColor(Color.white); // set color for the asteroid
...
public Asteroid createSplitAsteroid(double minVelocity,
 ...
 minVelocity,maxVelocity,hitsLeft-1,numSplit,classic1);
 }
...
public void draw(Graphics g){
 ...
 g.setColor(Color.white); //set color of flame
...
public void draw(Graphics g){
 g.setColor(Color.white); //set shot color
